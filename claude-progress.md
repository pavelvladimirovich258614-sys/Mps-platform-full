...
- Feature state: F01–F20 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F18 добавляет строгую `figure[data-carousel="images"]` карусель, F19 — её React NodeView preview и удаление отдельных изображений. F20 исправляет selection после upload: setImage и перевод позиции выполняются атомарно, впереди используется text selection, а на конце документа — GapCursor; повторная toolbar-загрузка больше не должна заменять предыдущий image-node. Drag-and-drop, paste, reorder и autoplay остаются будущим отдельным продуктовым scope. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но это не MPS feature gate.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; frontend F19 revision `d46d786` deployed с verified production VITE markers, served F19 JS/CSS и двумя успешными `deploy/smoke.sh`, backend оставался active без restart. Live editor/admin smoke подтвердил NodeView controls/delete и published carousel, но выявил закрытый локально в F20 repeated-upload selection defect; temporary article и пять media удалены. F20 production ещё не развёрнут. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`; nginx, certbot, HSTS, systemd timers и PostgreSQL backup проверены на VPS.
...
- Next best action: дождаться отдельного подтверждения на frontend-only production rollout F20; затем повторить live-сценарий двух toolbar uploads без End/ArrowRight и подтвердить editor/published carousel. Drag-and-drop, paste, reorder и autoplay не начинать без нового продуктового решения. Не менять Unisender transport: внешний сетевой блокер остаётся отдельно.
