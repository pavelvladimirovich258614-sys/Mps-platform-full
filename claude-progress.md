...
- Feature state: F01–F27 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F22 реактивно синхронизирует TipTap toolbar B/I/S с selection/transaction; F23 намеренно отключает наследование Bold/Italic/Strike на правой границе mark через `inclusive: false`, в отличие от Word/Google Docs. F24 даёт editor/admin доступ только к собственным drafts; чужие drafts скрыты 404, а draft→published через PATCH заполняет `published_at`. F25 расширяет media upload: HEIC/HEIF декодируются через `pillow-heif` и сохраняются как WebP, AVIF сохраняется как AVIF; это расширение форматов, а не PNG/JPEG-регрессия. F26 закрывает composer только после успешных POST/PATCH; при server error он остаётся открыт с ошибкой. F27 временно скрывает email login только во frontend флагом `EMAIL_LOGIN_ENABLED = false`; Telegram Login и backend email API сохранены. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F26 deployed at `2be15d5`. Rollback `/root/backups/mps-frontend-f26-20260824T133425Z` contains the prior frontend dist. Backend остался active без restart; frontend rebuilt with production VITE values, served asset returned 200, and `deploy/smoke.sh` passed. F25 production venv contains `pillow-heif==1.5.0` and its authorized media smoke remains recorded. F27 is local-only and awaits separate frontend deploy. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Auth/UI state: production build использует `https://mir.pod-solncem.ru/api/v1` и `Reg_Under_the_sun_bot`; закрыты найденные UI-проблемы login/profile (logout, avatar upload, золотой online-индикатор, toast поверх modal). F27 локально скрывает email form/copy, оставляя Telegram Login Widget единственным видимым guest path; callback contract подтверждён тестом. role storage устойчиво читает legacy `ADMIN` и текущие строчные значения, что подтверждено live callback 200.
...
- Next best action: отдельное approval Павла на production rollout F27 frontend; после deploy Павел подтверждает вход через Telegram, так как у Codex нет тестового Telegram account. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
