...
- Feature state: F01–F35 are `passing`; F36 is `in_progress`. F36 Packages 1–2 are production-deployed at `61ff1a5` and `6128c74`: forum topics/messages use opaque `id DESC` keyset pages (`items`/`next_cursor`, default 20, max 50), country-card counts use one `LEFT JOIN + GROUP BY`, SQL performs topic search, topic quota is serialized per user with `SELECT FOR UPDATE`, and verified JWT users are limited to 5 topic writes/minute and 10 message writes/minute with a Russian 429 response. Package 3 is locally complete and awaits separate deployment approval: authors/admins may delete topics or messages; topic deletion cascades; message deletion atomically decrements its topic counter and recalculates `last_message_at`. Package 4 (forum naming) remains unstarted. F35 полностью завершает личный кабинет: все пять вкладок PublicProfile используют реальные данные — Активность, Публикации, Ответы, Лайки и Подписки. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
...
- Next best action: obtain separate approval to deploy F36 Package 3, then continue Package 4 (forum naming). Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Deployment is explicitly unapproved. Package 3 has no migration, but a production rollout still requires the normal backup, pull, backend restart, frontend publish and smoke after separate approval.
