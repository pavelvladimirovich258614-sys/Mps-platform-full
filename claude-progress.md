...
- Next best action: create the authorised local F41 commit, then await separate push/deploy approval. Production activation needs a new BotFather relay token, a freshly generated TELEGRAM_WEBHOOK_SECRET in server .env, backend restart and setWebhook; do not run any of these before explicit approval. Do not start F38 duplicate-run/admin-settings work, F37 Session C/D or alter Unisender transport without a separate decision.
...
- F41 update: local webhook code is ready: `POST /api/v1/internal/telegram-webhook` verifies `X-Telegram-Bot-Api-Secret-Token` against a new fail-closed setting, parses manager/lawyer replies and persists the existing Question/Notification transition. tg_relay now logs and raises a token-free failure. Activation has not been performed.
