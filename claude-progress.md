...
- Feature state: F01–F26 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F22 реактивно синхронизирует TipTap toolbar B/I/S с selection/transaction; F23 намеренно отключает наследование Bold/Italic/Strike на правой границе mark через `inclusive: false`, в отличие от Word/Google Docs. F24 даёт editor/admin доступ только к собственным drafts; чужие drafts скрыты 404, а draft→published через PATCH заполняет `published_at`. F25 расширяет media upload: HEIC/HEIF декодируются через `pillow-heif` и сохраняются как WebP, AVIF сохраняется как AVIF; это расширение форматов, а не PNG/JPEG-регрессия. F26 закрывает composer только после успешных POST/PATCH; при server error он остаётся открыт с ошибкой. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F25 deployed at `e1a35f3`. Rollback `/root/backups/mps-f25-20260824T131202Z` contains prior revision/dist. Production venv installed `pillow-heif==1.5.0`, `mps-backend` restarted/health active, frontend rebuilt with production VITE values and served asset 200; `deploy/smoke.sh` passed. Live authorized media smoke: synthetic HEIC POST 200 and served as valid WebP, PNG/JPEG POST 200, renamed text file returned Russian 422, and exactly three temporary media files were cleaned. F24 draft evidence remains as previously recorded. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Next best action: отдельное approval Павла на production rollout F26 frontend; после deploy повторить live authenticated browser check клика по черновику. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
