...
- Next best action: await a separately confirmed task. F43 remains `open` only; do not start polling/refetch implementation without a plan and approval. Do not start F38 duplicate-run/admin-settings work, F37 Session C/D or alter Unisender transport without a separate decision.
...
- F43 update: `open`, not started. `useQA` does not poll/refetch an already-open modal after Telegram changes a Question externally; reopening remounts the hook and fetches current data. Candidate scope is conditional 15–30 second polling while open Questions exist, or a smaller open/focus refetch.
