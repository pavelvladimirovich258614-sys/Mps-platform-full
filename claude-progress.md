...
- Feature state: 56 tracker records; WIDG-1 and WIDG-2 are production-deployed `passing` together with `P0-POST-MEDIA` and WIDG-4. REV-2, P0-LIGHT-THEME, FEED-UX-SEO/FEED-F/FEED-G and About navigation remain production-deployed `passing`. F47 and F48c remain their pre-existing `in_progress` exceptions; F48a/F48b are production-deployed `passing`. `init.sh` still stops only on the known Git Bash Win32 Error 5/global pip-check conflicts, so complete MPS suites are verified directly outside that shared-environment failure.
- Deploy state: the completed WIDG-1/WIDG-2 application rollout reached `9586cafe0885941cfcfbd6c4f3bd634751e69680` on local `main`, `origin/main` and the VPS checkout before this documentation-only closeout. PostgreSQL is at Alembic `20260830_0020 (head)`; `mps-backend` is active/healthy. Production serves the WIDG-2 frontend with the required VITE values and no localhost API, and `deploy/smoke.sh` passed. WIDG-1 was deployed with its backend migration; WIDG-2 was frontend-only and did not restart backend.
...
- Next best actions: (1) назначить Сергея админом после получения от Павла точных email/Telegram ID и отдельного подтверждения production-role change; (2) отдельно диагностировать critical из `npm audit` (3 moderate, 1 high, 1 critical) через доказанный RED-контракт до любого фикса, не применять `npm audit fix --force` вслепую; (3) завтра отдельным промптом диагностировать, почему Telegram-уведомления от виджета Связио не доходят. Не начинать эти задачи в текущем closeout.
