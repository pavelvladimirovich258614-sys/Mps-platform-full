...
- Next best action: при желании Павел может подтвердить в живой Telegram-сессии статью с явно выбранной обложкой; no-cover ветка уже проверена на production и не создаёт ни картинки, ни fallback, ни пустой высоты. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
