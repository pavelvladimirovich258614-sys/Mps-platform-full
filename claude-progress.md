...
- Feature state: F01–F19 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F17 исправляет F03 media contract: `image.load()` принудительно декодирует изображение до записи, а Pillow `OSError`/`UnidentifiedImageError` возвращают `422 «Некорректное изображение»` вместо 500. F18 добавляет строгую `figure[data-carousel="images"]` карусель для двух и более соседних TipTap изображений. F19 добавляет React NodeView preview в composer, удаление отдельного изображения, понятную SVG image-иконку и убирает single-option publication type select; stored HTML и backend не меняются. Drag-and-drop, paste, reorder и autoplay остаются будущим отдельным продуктовым scope без выделенного feature ID. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но это не MPS feature gate.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F18 backend/frontend revision `6ab2e40` deployed from `61ebd31`, with a successful pre-deploy PostgreSQL backup, mps-backend restart/loopback health and deploy smoke. Remote frontend build has both production VITE markers and no localhost API; rollback dist is `/root/backups/mps-frontend-f18-20260823T221000Z`. Live temporary article proved the published carousel controls and separate-image behavior, then API DELETE 204/GET 404 and three media cleanup completed. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`; nginx, certbot, HSTS, systemd timers и PostgreSQL backup проверены на VPS.
...
- Next best action: после отдельного подтверждения выполнить frontend-only production rollout F19 и live editor/admin smoke. Drag-and-drop, paste, reorder и autoplay не начинать без нового продуктового решения. Не менять Unisender transport: внешний сетевой блокер остаётся отдельно.
