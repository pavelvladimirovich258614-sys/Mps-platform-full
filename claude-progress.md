...
- Next best action: after the approved local F43 commit, wait for separate push/deploy approval. Do not start F38 duplicate-run/admin-settings work, F37 Session C/D or alter Unisender transport without a separate decision.
...
- F43 update: `passing` locally, not deployed. A qa_answered click now resolves the linked Question, shows the correct manager/lawyer label, opens the corresponding Q&A tab, marks only that notification read and scrolls/highlights the exact thread. `useQA` polls every 30 seconds only while an open Question exists and stops after answer or unmount. RED target had 3 expected failures / 30 passed; GREEN target 33 passed; full frontend 129 passed; build succeeded.
