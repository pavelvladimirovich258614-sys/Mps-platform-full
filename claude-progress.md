...
- Feature state: F01–F35 are `passing`; F36 is `in_progress`. F36 Package 1 is locally complete: forum topics/messages use opaque `id DESC` keyset pages (`items`/`next_cursor`, default 20, max 50), country-card counts use one `LEFT JOIN + GROUP BY`, and SQL performs topic search. Migration `20260826_0013` adds the agreed forum indexes. Frontend accumulates pages through «Показать ещё». Packages 2–4 (concurrency/rate limiting, author/admin deletion, forum naming) remain unstarted; no F36 production deployment is approved. F35 полностью завершает личный кабинет: все пять вкладок PublicProfile используют реальные данные — Активность, Публикации, Ответы, Лайки и Подписки. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
...
- Next best action: F36 Package 2 — separately approved concurrency and rate-limiting work for forum writes. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
