...
- Feature state: F01–F24 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F24 даёт editor/admin доступ только к собственным drafts; чужие drafts скрыты 404, а draft→published через PATCH заполняет `published_at`. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F22 и F23 frontend rollouts применены после owner approval. F24 production не применялся и ожидает отдельного approval; он включает backend migration `20260824_0011_post_updated_at`, поэтому перед deploy требуется штатный Alembic upgrade и restart backend. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB и русский 422 не менялись. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
