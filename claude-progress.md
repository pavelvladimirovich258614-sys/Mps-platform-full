...
- Feature state: F01–F35 are `passing`. F35 полностью завершает личный кабинет: все пять вкладок PublicProfile используют реальные данные — Активность, Публикации, Ответы, Лайки и Подписки. Activity API использует opaque `(created_at, id)` keyset pagination и batch context resolution; UI имеет четыре типа событий и «Показать ещё». ActivityLog records post_published, comment_created, post_liked and user_followed, removes reversible like/follow events atomically. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
...
- Next best action: read-only diagnosis of slow personal-profile loading (F32 from the original findings list). Do not change Unisender transport without a separate decision: the external network blocker remains separate.
