...
- Feature state: F01–F35 are locally `passing`. Sessions 1–3 are production-deployed; Session 4a Activity infrastructure is also deployed at `2e58222`. Session 4b adds the public profile Activity API/UI: opaque `(created_at, id)` keyset pagination, batched context resolution, owner/public visibility filtering, «Показать ещё» and the agreed exact empty state. ActivityLog records post_published, comment_created, post_liked and user_followed, removes reversible like/follow events atomically. Public likes remain visible to every profile visitor; Likes DTO has `liked_at`, cards render an optional real cover/date, and a same-tab post toggle refreshes App's shared current-user likes cache. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F35 Sessions 1–3 are deployed through `c12e102`. Session 4a `2e58222` was production-deployed with a fresh PostgreSQL backup `/root/backups/mps-activity-log-pre-20260825T030157Z.dump`; migration `20260825_0012` backfilled 4 activity rows, backend readiness and `deploy/smoke.sh` passed. Session 4b is locally verified and awaits separate production deployment approval. F25 production venv contains `pillow-heif==1.5.0`; nginx permits 11m, backend raw-file limit is 10 MiB. Migration `20260825_0012` is applied on PostgreSQL; `comments_moderation_enabled=false`. MPS uses separate PostgreSQL DB/role, Redis DB 2 and backend on `127.0.0.1:8001`.
...
- Next best action: obtain separate approval to deploy F35 Session 4b (backend and frontend); do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Evidence recorded: F35 is locally `passing`; Session 4b has not been production-deployed and awaits separate approval.
- Commits: pending local checkpoint.
...
- Next best action: obtain separate approval for the F35 Session 4b production rollout.
