...
- Feature state: F01–F34 passing. F35 remains in_progress: Sessions 1–3 are deployed; Session 4a Activity infrastructure is locally complete awaiting deployment. ActivityLog records post_published, comment_created, post_liked and user_followed, removes reversible like/follow events atomically, and has no read API/UI yet. Public likes remain visible to every profile visitor; Likes DTO has `liked_at`, cards render an optional real cover/date, and a same-tab post toggle refreshes App's shared current-user likes cache. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F35 Session 3 revision `c12e102` was deployed from `72ce494`, mps-backend restart reached readiness on attempt 2, and remote frontend build plus `deploy/smoke.sh` passed. Backend/frontend rollback copies are `/root/backups/mps-backend-f35-s3-20260825T014224Z.tgz` and `/root/backups/mps-frontend-f35-s3-20260825T014224Z`; served `index-DL7pFkN2.js` and `GET /users/1/likes` both returned 200. F25 production venv contains `pillow-heif==1.5.0`; nginx permits 11m, backend raw-file limit is 10 MiB. Migration `20260824_0011` is applied on PostgreSQL; `comments_moderation_enabled=false`. MPS uses separate PostgreSQL DB/role, Redis DB 2 and backend on `127.0.0.1:8001`.
...
- Next best action: deploy F35 Session 4a only after separate approval, then implement the separately approved Session 4b Activity read API/UI. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Evidence recorded: F35 remains `in_progress`; no Activity endpoint/frontend was added and Session 4a production deployment requires separate approval.
...
- Next best action: obtain separate deployment approval for Session 4a, then plan Session 4b read API/UI.
