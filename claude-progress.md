...
- Feature state: F01–F34 passing. F35 remains in_progress: Session 1 locally completed the public-profile subscriptions lists — `GET /users/{id}/followers` and `/following` expose only public users with per-viewer `is_following`, and PublicProfile renders both lists with a local follow toggle. Existing `GET /posts?author_id=` remains the regression baseline for Publications. The next F35 session is Responses (the user's own comments); Likes remain as existing liked posts, while Activity needs a separate product decision. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
...
- Next best action: F35 Session 2 — add the user's own-comments «Ответы» list with the agreed empty text «Пока нет ответов. Ваши ответы появятся здесь.»; do not alter current Likes. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Evidence recorded: F35 remains `in_progress`; no production deployment was performed.
