...
- Feature state: F01–F34 passing; полный рабочий цикл F15–F34 завершён и находится в текущем production revision. F29 синхронизирует avatar picker с F25 MIME-набором и очищает input для повторного выбора. F30 добавляет подтверждённое удаление черновика из списка. F31 вводит явный `cover_url` end-to-end с приоритетом над inline-изображениями; F32 исправил одновременный gradient+cover render; F33 окончательно удалил fallback «Под солнцем», поэтому статья без cover не создаёт верхний cover DOM. F34 сохраняет presence-семантику `last_seen_at` за последние 120 секунд без WebSocket: `/online` возвращает реальные avatar_url, зелёная точка закреплена на аватаре в сайдбаре и public profile, polling происходит после auth и далее каждые 30 секунд с cleanup таймера. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; current F34 frontend revision `3451397`, VPS fast-forwarded from `e7e97b7`, rollback `/root/backups/mps-frontend-f34-20260824T172052Z`, served `index-C-CVCK1W.js` returned 200 and `deploy/smoke.sh` passed; `mps-backend` stayed active without restart. F31 backend+frontend rollout reached `9bc70d4`, backend restarted/readiness green. F25 production venv contains `pillow-heif==1.5.0`; nginx permits 11m, backend raw-file limit is 10 MiB. Migration `20260824_0011` is applied on PostgreSQL; `comments_moderation_enabled=false`. MPS uses separate PostgreSQL DB/role, Redis DB 2 and backend on `127.0.0.1:8001`.
...
- Next best action: F35 is not started: replace personal-cabinet placeholder tabs (Активность/Публикации/Ответы/Лайки/Подписки) with real data and complete the «Подписаться» action in the subscribers list. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
