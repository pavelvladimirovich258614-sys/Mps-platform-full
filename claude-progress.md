...
- Feature state: F01–F23 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F23 задаёт осознанный platform UX: B/I/S не наследуются справа от mark, но сохраняются внутри фрагмента. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F22 frontend-only rollout применён на VPS SHA `1a680db`, `deploy/smoke.sh` прошёл, backend не перезапускался и active. F23 production не применялся и ожидает отдельного approval. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB и русский 422 не менялись. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
