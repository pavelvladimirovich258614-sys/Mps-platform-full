...
- Feature state: F01–F16 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F16 добавляет единичную загрузку JPEG/PNG/WebP через TipTap toolbar: существующий POST `/api/v1/media` возвращает URL, который вставляется в текущую selection; ошибка остаётся в composer. Drag-and-drop, paste и карусель не входят в F16. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но это не MPS feature gate.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F16 frontend-only production revision `7a793f0`. Backend diff F15→F16 пуст, `mps-backend` active и не перезапускался; remote production Vite build/smoke и guest published-image render прошли. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. Неисправленное F03-расхождение: corrupted bytes с MIME image/png дают 500 вместо документированного 422; invalid MIME даёт ожидаемый 422. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`; nginx, certbot, HSTS, systemd timers и PostgreSQL backup проверены на VPS.
...
- Next best action: F14 Phase 3 — multi-image carousel, только после отдельного продуктового решения. Не менять Unisender transport: внешний сетевой блокер остаётся отдельно. F16 production deploy ожидает отдельного подтверждения владельца.
