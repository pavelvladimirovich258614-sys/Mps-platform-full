...
- Feature state: F01–F14 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F14 полностью на production: TipTap composer открывается modal, Bold-space regression исправлен штатным `onUpdate`; серия UI-фиксов включает общий подзаголовок, исключение `fishka` из composer, единый заголовок «Статьи» и CTA после комментариев. Премодерация комментариев настраивается key-value setting `comments_moderation_enabled`, default `false`; editor/admin меняет её через `PATCH /admin/settings`. UI лайков подключён в карточке и полной статье, локальный toggle обновляет счётчик без reload, guest открывает login modal. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но это не MPS feature gate.
...
- Next best action: вечерний backlog — frontend-редактирование/удаление опубликованной статьи, затем image upload в composer (F14 Phase 2), затем multi-image carousel (Phase 3). Не менять Unisender transport: внешний сетевой блокер остаётся отдельно.
