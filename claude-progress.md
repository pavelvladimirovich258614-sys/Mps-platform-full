...
- Feature state: F01–F22 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. Цикл F15–F22 закрыт: редактирование/удаление статей, image upload, 422 для повреждённых изображений, безопасная карусель, editor NodeView и UX-полировка, hotfix последовательной вставки, ingress/leading-carousel fix и реактивный TipTap toolbar state. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но полные MPS suites были пройдены отдельно и зелёные.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; local `main`, `origin/main` и VPS синхронизированы на `ada1f52`. F21 применён: активный MPS nginx server block содержит `client_max_body_size 11m`, backend raw-file limit 10 MiB и русский 422 не менялись; nginx прошёл `nginx -t` и был graceful-reload. Production frontend собран с проверенными VITE-переменными, served bundle не содержит localhost API; `deploy/smoke.sh` зелёный. Backend не перезапускался и остался active. Live editor/admin smoke подтвердил upload двух файлов >1 MiB с 200 без 413, единственную leading-карусель, опубликованный carousel Next/Prev и удаление временной статьи/media. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
