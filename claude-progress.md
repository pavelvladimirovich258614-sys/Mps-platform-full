...
- Auth/UI state: production build использует `https://mir.pod-solncem.ru/api/v1` и `Reg_Under_the_sun_bot`; F27 production скрывает email form/copy, оставляя Telegram Login Widget единственным видимым guest path. F28 локально добавляет выход из own public profile через •••; visitor profile его не получает. role storage устойчиво читает legacy `ADMIN` и текущие строчные значения, что подтверждено live callback 200.
...
- Next best action: отдельное approval Павла на production rollout F28 frontend; после deploy подтвердить в живой Telegram-сессии owner-only logout из публичного профиля. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
