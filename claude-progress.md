...
- Feature state: F01–F18 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F17 исправляет F03 media contract: `image.load()` принудительно декодирует изображение до записи, а Pillow `OSError`/`UnidentifiedImageError` возвращают `422 «Некорректное изображение»` вместо 500. F18 добавляет строгую `figure[data-carousel="images"]` карусель для двух и более соседних TipTap изображений, с доступными previous/next/dot controls; одиночный img не меняется. Drag-and-drop, paste, reorder и autoplay вынесены в будущий F19. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но это не MPS feature gate.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; backend revision `35f6914` включает F17, frontend F16 remains `7a793f0`. VPS fast-forwarded ca0880f→35f6914; mps-backend restart readiness green on attempt 2 and smoke passed. Live corrupted image/png now returns 422 without a media file; JPEG/PNG/WebP remain 200 and test artifacts were cleaned. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`; nginx, certbot, HSTS, systemd timers и PostgreSQL backup проверены на VPS.
...
- Next best action: F19 advanced image authoring (drag-and-drop, paste, reorder, autoplay) — только после отдельного продуктового решения. Не менять Unisender transport: внешний сетевой блокер остаётся отдельно.
...
- Evidence recorded: F18 marked passing; production intentionally unchanged and awaits a separate owner approval. F19 is only a future scope for drag-and-drop, paste insertion, reorder and autoplay.
- Next best action: approve a separate production rollout for F18, or separately scope F19; do not start F19 automatically.
