...
- Feature state: F01–F25 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F22 реактивно синхронизирует TipTap toolbar B/I/S с selection/transaction; F23 намеренно отключает наследование Bold/Italic/Strike на правой границе mark через `inclusive: false`, в отличие от Word/Google Docs. F24 даёт editor/admin доступ только к собственным drafts; чужие drafts скрыты 404, а draft→published через PATCH заполняет `published_at`. F25 расширяет media upload: HEIC/HEIF декодируются через `pillow-heif` и сохраняются как WebP, AVIF сохраняется как AVIF; это расширение форматов, а не PNG/JPEG-регрессия. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F22–F24 deployed. F24 production revision `9872364`: перед миграцией создан и прочитан PostgreSQL dump; Alembic `20260822_0010 → 20260824_0011` применён, `mps-backend` restarted/health 200/active. Frontend rebuilt с проверенными production VITE variables, served F24 asset возвращает 200, `deploy/smoke.sh` passed. Live authorized API smoke: own draft POST 201, list/detail 200, PATCH publish 200, post in public feed, cleanup DELETE 204. Foreign-draft 404 подтверждён тестами, но не live: на production был только один editor/admin. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB и русский 422 не менялись. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Next best action: отдельное approval Павла на production rollout F25; не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
