...
- Feature state: F01–F21 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F21 поднимает nginx ingress limit в template до 11m при неизменном backend raw-file limit 10 MiB и меняет composer UX: новый img всегда вставляется в начало документа, а непрерывная начальная media-группа нормализуется в одну карусель с новым кадром в конце. Drag-and-drop, paste, reorder и autoplay остаются будущим отдельным продуктовым scope. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но отдельные полные MPS suites зелёные.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F20 revision `aac5536` синхронизирована с origin/main и VPS, production frontend build/smoke зелёные. Live editor/admin F20 smoke подтвердил две и три последовательные toolbar-загрузки без движения курсора, одну editor/published карусель и переключение трёх разных кадров; temporary article и media удалены. F21 production ещё не развёрнут: активный MPS nginx server block всё ещё использует default client body limit 1m, поэтому валидные media >1 MiB получают HTML 413 до FastAPI. Миграция `20260822_0010` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Next best action: дождаться отдельного подтверждения на push и production rollout F21. Rollout должен применить новый nginx template с обязательными backup/`nginx -t`/reload и пересобрать frontend с проверенными VITE-переменными; затем live smoke двумя файлами, включая валидный PNG >1 MiB, должен дать два 200 и одну начальную карусель. Drag-and-drop, paste, reorder и autoplay не начинать без нового продуктового решения. Не менять Unisender transport: внешний сетевой блокер остаётся отдельно.

## Session Record

### Session 50 — 2026-08-24 (Codex, F21 media ingress and leading carousel)
- Goal: устранить nginx 413 при повторной загрузке допустимого изображения и сделать единую media-группу обязательным началом TipTap-документа независимо от selection.
- Diagnosis before code: тем же production access-token первый PNG 84 B вернул 200, второй валидный PNG 7 692 467 B — HTML 413 Request Entity Too Large. Backend journal увидел только первый 200 и не содержал ошибок; nginx logs подтвердили reject до FastAPI для нескольких bodies 1.9–5.8 MB. Применимый MPS server block не имел client_max_body_size; file input reset, JWT и rate limit исключены. Диагностический media очищен, backend остался active.
- Completed: `deploy/nginx.conf` теперь допускает 11m multipart request, оставляя backend raw-file validation 10 MiB без изменений. RichTextEditor вместо setImage по selection собирает только непрерывную начальную img/imageCarousel-группу: при её отсутствии вставляет одиночный img в позицию 0, иначе flatten+append создаёт одну карусель; текст остаётся ниже. Dependencies, database, sanitizer и media endpoint не менялись.
- Verification run: RED nginx — 1 failed / 2 passed из-за отсутствующего directive; backend media regression — 7 passed и два последовательных upload одним token получили 200. RED frontend — 3 failed / 6 passed на middle/end/repeated-at-end порядке. GREEN frontend — 9 passed; GREEN backend/deploy — 10 passed; related frontend — 3 files / 17 passed. Full backend — 65 passed in 19.54s; full frontend — 15 files / 73 passed; build — 114 modules, success. Final `./init.sh` остановился до MPS tests только на известном внешнем Hermes pip check missing charset-normalizer.
- Evidence recorded: F21 marked passing in feature_list.json; clean-state checklist and handoff updated. Production intentionally unchanged pending separate approval.
- Commits: будет создан локальный `F21: media ingress и начальная карусель [passing]`; push не выполняется без отдельного подтверждения владельца.
- Known risks: production nginx продолжает возвращать 413 для файлов >1 MiB до отдельного rollout; template нужно применять с backup, `nginx -t` и reload. Vite сохраняет прежний warning о chunk >500 kB.
- Next best action: после явного approval push F21, безопасно применить nginx config/reload, пересобрать frontend и live-проверить два последовательных upload, включая файл >1 MiB, затем удалить временную статью/media.
