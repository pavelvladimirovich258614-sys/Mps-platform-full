...
- Feature state: F01–F31 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. F22 реактивно синхронизирует TipTap toolbar B/I/S с selection/transaction; F23 намеренно отключает наследование Bold/Italic/Strike на правой границе mark через `inclusive: false`, в отличие от Word/Google Docs. F24 даёт editor/admin доступ только к собственным drafts; чужие drafts скрыты 404, а draft→published через PATCH заполняет `published_at`. F25 расширяет media upload: HEIC/HEIF декодируются через `pillow-heif` и сохраняются как WebP, AVIF сохраняется как AVIF; это расширение форматов, а не PNG/JPEG-регрессия. F26 закрывает composer только после успешных POST/PATCH; при server error он остаётся открыт с ошибкой. F27 временно скрывает email login только во frontend флагом `EMAIL_LOGIN_ENABLED = false`; Telegram Login и backend email API сохранены. F28 добавляет owner-only «Выйти» в существующее ••• меню публичного профиля, переиспользуя `auth.logout()` и redirect в guest feed. F29 синхронизирует avatar picker с F25 (HEIC/HEIF/AVIF) и сбрасывает file input для повторного выбора того же файла. F30 даёт редактору отдельное подтверждаемое удаление черновика прямо из списка. F31 подключает явный Post.cover_url end-to-end: выбор и preview в composer, API DTO и отображение в ленте/статье; без URL fallback сохранён. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F30 deployed at `11dff37` (frontend-only), rollback `/root/backups/mps-frontend-f30-20260824T152645Z`, smoke and served-bundle marker passed; backend remained active. F29 deployed at `629f824` (frontend-only). F28 deployed at `df2cb6b` (frontend-only), rollback `/root/backups/mps-frontend-f28-20260824T142351Z`, `deploy/smoke.sh` passed and backend remained active. F27 deployed at `6a02ddd` (frontend-only): live login UI shows only Telegram Login, without email field. F31 is local-only and awaits separate backend+frontend deploy approval. F25 production venv contains `pillow-heif==1.5.0` and its authorized media smoke remains recorded. Активный MPS nginx server block содержит `client_max_body_size 11m`; backend raw-file limit 10 MiB. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Auth/UI state: production build использует `https://mir.pod-solncem.ru/api/v1` и `Reg_Under_the_sun_bot`; F27 production скрывает email form/copy, оставляя Telegram Login Widget единственным видимым guest path. F28 production добавляет выход из own public profile через •••; visitor profile его не получает. F29 локально расширяет avatar picker современными форматами и разрешает повторный выбор того же файла. role storage устойчиво читает legacy `ADMIN` и текущие строчные значения, что подтверждено live callback 200.
...
- Next best action: отдельное approval Павла на production rollout F31 backend+frontend; после deploy подтвердить в живой Telegram-сессии upload/preview/edit cover and feed/article rendering. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
...
- Commits: local F31 completion commit created; production push remains unapproved.
- Next best action: await explicit F31 backend+frontend deploy approval; then perform live authenticated cover upload, edit and display verification.
