...
- Feature state: F01–F34 passing. F34 сохраняет presence-семантику `last_seen_at` за последние 120 секунд без WebSocket: список `/online` теперь показывает реальный `avatar_url` или прежний gradient fallback, а зелёная точка закреплена снизу-справа на аватаре в сайдбаре и публичном профиле. После auth список обновляется сразу и затем каждые 30 секунд с cleanup таймера; header продолжает получать новый avatar из состояния `useAuth` после PATCH `/me`. F31 подключает явный `Post.cover_url` end-to-end; F32 отделяет реальную картинку от fallback-контейнера; F33 по финальному продуктовому решению полностью удаляет fallback «Под солнцем». Теперь `cover_url` даёт единственную самостоятельную обложку, а без него cover DOM и зарезервированной высоты нет. Inline TipTap media остаются содержимым body и не становятся обложкой автоматически. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F31 backend+frontend rollout достиг `9bc70d4`, backend перезапущен/readiness green. F33 frontend-only rollout развёрнут из `e4c302f`; rollback `/root/backups/mps-frontend-f33-20260824124812`, served bundle `index-CXtH547q.js`, `deploy/smoke.sh` passed, `mps-backend` оставался active и не перезапускался. F25 production venv содержит `pillow-heif==1.5.0`; nginx допускает 11m, backend raw-file limit 10 MiB. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Next best action: F34 ожидает отдельного approval на frontend-only production deploy; после него Павел может проверить в Telegram-сессии аватары и online indicator. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
