...
- Feature state: F01–F32 passing. F31 подключает явный `Post.cover_url` end-to-end: composer picker/preview, API DTO и отображение в ленте/статье. F32 устраняет остаточный fallback-контейнер: при непустом URL рендерится самостоятельная обложка, при его отсутствии — только «Под солнцем». Inline TipTap media остаются содержимым body и не становятся обложкой автоматически. `init.sh` использует `python -m pip`; его global Python pre-flight остаётся внешним Hermes/desktop blocker, поэтому полные MPS suites проверяются отдельно.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F31 backend+frontend rollout достиг `9bc70d4`, backend перезапущен/readiness green. F32 frontend-only hotfix развёрнут из `02823b9`; rollback `/root/backups/mps-frontend-f32-20260824121451`, served bundle `index-BGDRzZT7.js`, `deploy/smoke.sh` passed, `mps-backend` оставался active. F25 production venv содержит `pillow-heif==1.5.0`; nginx допускает 11m, backend raw-file limit 10 MiB. Миграция `20260824_0011` применена на PostgreSQL; `comments_moderation_enabled=false`. MPS использует отдельные PostgreSQL DB/role, Redis DB 2 и backend на `127.0.0.1:8001`.
...
- Next best action: Павлу подтвердить в живой Telegram-сессии создание статьи с явной обложкой: наверху ленты и статьи должна быть только картинка, без «Под солнцем»; статья без обложки должна сохранить fallback. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
