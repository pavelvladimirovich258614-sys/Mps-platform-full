...
- Feature state: F01–F34 passing. F35 remains in_progress: Sessions 1–2 are locally complete. Existing `GET /posts?author_id=` remains the Publications regression baseline; subscriptions are deployed. Replies now use `GET /users/{id}/comments`: only the profile owner sees all own statuses, while visitors receive only approved comments; frontend status labels require an admin viewing their own profile. Likes remain as existing liked posts, while Activity needs a separate product decision. Session 2 awaits separate deployment approval. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
- Deploy state: платформа развёрнута и живая на `https://mir.pod-solncem.ru`; F35 Session 1 revision `fe00787` deployed from `04ae21d`, mps-backend was restarted and readiness was green on attempt 2. Frontend rollback is `/root/backups/mps-frontend-f35-20260825T002954Z`; served `index-BH60U7HM.js` returned 200 with F35 markers and production API only, and `deploy/smoke.sh` passed. Migration check made no schema changes. F25 production venv contains `pillow-heif==1.5.0`; nginx permits 11m, backend raw-file limit is 10 MiB. Migration `20260824_0011` is applied on PostgreSQL; `comments_moderation_enabled=false`. MPS uses separate PostgreSQL DB/role, Redis DB 2 and backend on `127.0.0.1:8001`.
...
- Next best action: deploy F35 Session 2 only after separate approval, then proceed to the remaining F35 scope; do not alter current Likes. Activity still requires a separate product decision. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Evidence recorded: F35 remains `in_progress`; production deployment was not performed and requires separate approval.
