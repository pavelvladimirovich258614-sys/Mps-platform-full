...
- Feature state: F01–F21 passing; все три этапа F09 (`F09a1`, `F09a2`, `F09b`) passing. Цикл F15–F21 закрыт: редактирование/удаление статей, image upload, 422 для повреждённых изображений, безопасная карусель, editor NodeView и UX-полировка, hotfix последовательной вставки и ingress/leading-carousel fix. Drag-and-drop, paste, reorder и autoplay не начинались и остаются отдельным будущим product scope. `./init.sh` остаётся заблокирован внешним Hermes `pip check`, но полные MPS suites были пройдены отдельно и зелёные.
