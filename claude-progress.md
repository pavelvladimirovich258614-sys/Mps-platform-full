...
- Next best action: await separate approval to commit/deploy F37 Session B, or plan Session C (admin settings UI) separately. Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- RED→GREEN: `App.routing.test.tsx` RED had 3 expected missing-form/permission failures; GREEN — 27 passed. Full frontend — 19 files / 121 passed. `npm run build` — success, 116 modules; only the standard Vite chunk-size warning. Backend, migrations, environment and production were untouched. F37 remains `in_progress`; Session B is uncommitted and undeployed.
