...
- Feature state: F01–F44 are `passing`. F37 Sessions A (`4f86725`) and B (`df36dc2`) are production-deployed; Session C is passing locally with an admin-only `/fishki` toggle for `fishka_submissions_enabled` and awaits separate push/deploy approval. The separately deferred 160-item import remains unstarted Session D. F38–F44 are production-deployed: F38 Packages 1–3 at `9d18156`, `a97327c`, `21e55ac`; F39 at `e688773`; F40 at `4f868ef`; F41 at `1782b5a`; F42 at `3d6ac1c`; F43 at `2ffb60d`; F44 at `0e49bbe`. Deferred duplicate-run protection/admin UI for Иришка are future enhancements. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
- Deploy state: platform is live at `https://mir.pod-solncem.ru` on revision `0e49bbe`. PostgreSQL is at Alembic `20260826_0015 (head)`; `mps-backend` is active/healthy on `127.0.0.1:8001`; production frontend bundle is `/assets/index-Bg-pAkQQ.js` with the production API/bot markers and no localhost API. F44 rollback points are PostgreSQL `/var/backups/mps/mps-2026-08-26-080806.dump.gz`, backend `/root/backups/mps-backend-f44-20260826T120806Z.tgz`, and frontend `/root/backups/mps-frontend-f44-20260826T120806Z`; final `deploy/smoke.sh` passed. MPS retains its isolated PostgreSQL role/database and Redis DB 2.
...
- Next best action: keep the completed F37 Session C commit local and wait for separate push/deploy approval. Do not start the deferred F37 Session D import, Иришка duplicate-run/admin-settings work, forum N+1 optimization, npm dependency remediation or Unisender transport changes without a separate plan and approval.

- F37 update: `passing`. Sessions A/B are deployed; Session C is completed locally. Only an authenticated admin on `/fishki` loads GET `/admin/settings` and sees the toggle; a change PATCH-es `fishka_submissions_enabled`, waits for completion and preserves the previous state with a Russian error if the request fails. Editor and non-staff clients do not request the admin endpoint. RED — 1 expected failure / 32 passed; GREEN target — 34 passed; full frontend — 21 files / 137 passed; relevant backend regression — 14 passed; build — 117 modules. Session D remains separately deferred.
