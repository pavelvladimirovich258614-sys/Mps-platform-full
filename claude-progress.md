...
- Next best action: отдельное approval Павла на production rollout F30 frontend; после deploy подтвердить удаление черновика в живой Telegram-сессии. Отдельно выбрать product approach для article cover: автоматически брать первое inline-image либо реализовать явный cover_url flow. Не менять Unisender transport без отдельного решения: внешний сетевой блокер остаётся отдельно.
