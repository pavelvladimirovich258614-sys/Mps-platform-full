...
- Feature state: F01–F35 are `passing`; F36 is `in_progress`. F36 Package 1 is production-deployed at `61ff1a5`: forum topics/messages use opaque `id DESC` keyset pages (`items`/`next_cursor`, default 20, max 50), country-card counts use one `LEFT JOIN + GROUP BY`, SQL performs topic search, and migration `20260826_0013` adds the agreed indexes. Package 2 is locally complete: topic quota is serialized per user with `SELECT FOR UPDATE`; API and Иришка use atomic SQL increments for `messages_count`; verified JWT users are limited to 5 topic writes/minute and 10 message writes/minute with a Russian 429 response. Packages 3–4 (author/admin deletion, forum naming) remain unstarted; Package 2 production deployment requires separate approval. F35 полностью завершает личный кабинет: все пять вкладок PublicProfile используют реальные данные — Активность, Публикации, Ответы, Лайки и Подписки. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
...
- Next best action: obtain separate approval to deploy F36 Package 2, then continue Package 3 (author/admin deletion). Do not change Unisender transport without a separate decision: the external network blocker remains separate.
...
- Evidence recorded: Package 1 rollout is now recorded as production-deployed at `61ff1a5` (backup, Alembic `20260826_0013`, backend health, frontend publish and smoke passed). Package 2 remains local-only; F36 stays `in_progress` until Packages 3–4.
