...
- Feature state: F01–F44 are `passing`. F37 Sessions A (`4f86725`), B (`df36dc2`) and C (`b2b41fb`) are production-deployed. Session D is complete locally: nullable fishka categories, category API/filter UI, 11 additional exact Unicode emoji choices and a validated idempotent 160-item importer. Session D code, migration and frontend are not pushed/deployed; the real production import remains a separately approved post-deploy action. F38–F44 are production-deployed: F38 Packages 1–3 at `9d18156`, `a97327c`, `21e55ac`; F39 at `e688773`; F40 at `4f868ef`; F41 at `1782b5a`; F42 at `3d6ac1c`; F43 at `2ffb60d`; F44 at `0e49bbe`. Deferred duplicate-run protection/admin UI for Иришка are future enhancements. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so complete MPS suites are verified separately.
- Deploy state: platform is live at `https://mir.pod-solncem.ru` on revision `b2b41fb`. PostgreSQL remains at Alembic `20260826_0015 (head)` because Session D was not deployed; `mps-backend` remained unchanged during the Session C frontend rollout and is active/healthy on `127.0.0.1:8001`. The served Session C frontend bundle is `/assets/index-6rYQVEbg.js` with production API/bot markers and no localhost API; rollback is `/root/backups/mps-frontend-f37-c-20260827T135639Z`, and `deploy/smoke.sh` passed. MPS retains its isolated PostgreSQL role/database and Redis DB 2.
...
- Next best action: wait for explicit approval to push and deploy the completed F37 Session D commit. Deployment must take a fresh PostgreSQL backup, apply migration `20260827_0016`, restart the changed backend and rebuild frontend; the real `--apply` import of 160 production rows remains a separate explicit step after code deployment. Do not start Иришка duplicate-run/admin-settings work, forum N+1 optimization, npm dependency remediation or Unisender transport changes without a separate plan and approval.

- F37 update: `passing`. Sessions A/B/C are deployed; Session D is complete locally. `Post.category` is nullable, published fishki expose/filter by category, `/posts/fishki/categories` drives the `/fishki` topic dropdown, and the picker retains visual Unicode variants while avoiding exact duplicates. The versioned source parses as 160 contiguous records in 13 blocks. Import requires an explicit Pavel author ID, deterministic slugs and mandatory dry-run/apply mode; it creates no synthetic activity events. D1 RED was the expected missing importer and GREEN was 13 passed. D2 RED was 2 failures / 34 passed and GREEN was 36 passed. Isolated migration upgrade/downgrade/re-upgrade and dry-run/160 insert/160 unchanged rerun passed. Full backend — 119 passed/3 skipped; frontend — 21 files/139 passed; build — 117 modules. Production import was not run.

- F38 update: `passing`; Packages 1 (`9d18156`), 2 (`a97327c`) and 3 (`21e55ac`) are production-deployed. Package 3 provides direct non-persistent «Иришка ИИ» Q&A with keyword retrieval over the supplied 248-record knowledge base, shared MiniMax timeout/retry transport, and a 10/minute verified-user limit. Duplicate-run protection and admin settings UI are explicitly deferred future work.
