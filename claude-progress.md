...
- Next best action: F47 is optional because profiling found no N+1; either add only a test guard or close it with the recorded no-regression evidence. Then implement F48a, F48b with editor moderation, and F48c with the email form honestly disabled while network delivery is blocked. Web design remains last.
