...
- Next best action: follow the agreed remaining backlog order only after a new plan/approval: F46 admin UI for Иришка settings → F47 forum N+1 → decide with Pavel whether F48 a/b/c audit of Drafts/Reviews/Subscription is still needed → web design last. F46 and F47 are open, not `in_progress`; F48 a/b/c has not been audited. Whole-site search remains a separate discussed but unstarted backlog item. Email networking, npm audit and restoration of the missing skill files remain separate maintenance scopes.
