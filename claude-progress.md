...
- Feature state: all 47 tracked records covering F01–F45 are `passing` with evidence; historical F09 is split into F09a1/F09a2/F09b. F37 Sessions A (`4f86725`), B (`df36dc2`), C (`b2b41fb`) and D (`9ab7b0e`) are production-deployed. Session C embeds the admin-only `fishka_submissions_enabled` toggle in `/fishki`; Session D provides nullable categories, the dynamic «Тема» filter, 11 additional exact Unicode emoji choices and the validated idempotent 160-item importer. The approved production import created 160 records in 13 categories; the approved data operation removed the 15 exact records from `Реальные кейсы Сергея (главное)`, leaving 145 imported fishki, 146 fishki total and 12 categories. F45 is production-deployed with PostgreSQL advisory locking, a final row-locked recheck and partial unique index migration `20260828_0017`. F38–F44 remain production-deployed. `init.sh` uses `python -m pip`; its global Python pre-flight remains an external Hermes/desktop blocker, so MPS verification is recorded separately.
- Deploy state: production application revision is `a5186bce67c107dd8912f39361fa15b7fb637351`; local/origin/VPS were synchronized on that application checkpoint before this tracker-only follow-up. PostgreSQL is at Alembic `20260828_0017 (head)`, `mps-backend` is active/healthy and `deploy/smoke.sh` passed. F45 rollback is `/var/backups/mps/mps-2026-08-27-123301.dump.gz`, SHA-256 `3ac9b6d2cfca55f97bb83d549c7d4896c99011ec3f0a293567e49e6054325043`. The dynamic fishka category API/live dropdown expose 12 categories and exclude `Реальные кейсы Сергея (главное)`. The documentation checkpoint requires no production deployment; MPS retains its isolated PostgreSQL role/database and Redis DB 2.
