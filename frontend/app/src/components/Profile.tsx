...
export function Profile({ user, onClose, onRequestCode, onVerifyCode, onTelegramLogin, onUpdate, onUploadAvatar, onLogout, onError }: ProfileProps) {
  ...
  const uploadAvatar = async (event: React.ChangeEvent<HTMLInputElement>) => { const [file] = Array.from(event.target.files ?? []); event.currentTarget.value = ""; if (!file) return; try { await onUploadAvatar(file); } catch (cause) { onError(cause instanceof Error ? cause.message : "Не удалось загрузить аватар"); } };
  const logout = async () => { try { await onLogout(); onClose(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Не удалось выйти"); } };

  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Мой профиль" onMouseDown={onClose}><section className="profile-modal" onMouseDown={(event) => event.stopPropagation()}><h2>Мой профиль</h2><div className="profile-avatar-row"><span className="profile-avatar">{user.avatar_url && <img src={user.avatar_url} alt="" />}</span><div><strong>{user.name || "Путешественник"}</strong><small className="profile-role">Роль: {user.role}</small></div><label className="avatar-upload">Загрузить аватар<input aria-label="Загрузить аватар" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,image/avif" onChange={uploadAvatar} /></label></div><div className="form-grid"><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Имя" /><textarea value={bio} onChange={(event) => setBio(event.target.value)} placeholder="О себе" /></div><button className="online-toggle" onClick={() => setAnonymous(!anonymous)}><span>Показывать меня онлайн</span><i className={!anonymous ? "on" : ""}><b /></i></button><button className="primary-button" onClick={save}>Готово</button><button className="profile-logout" onClick={logout}>Выйти</button></section></div>;
}
