...
function Topics({ country, forum, onSelect, onError }: { country: Country; forum: ReturnType<typeof useForum>; onSelect: (topic: Topic) => void; onError: (message: string) => void }) {
  ...
  return <section className="pending-section"><div className="section-heading"><h2>{countryFlag(country.name)} Темы: {country.name}</h2></div>{forum.topics.loading ? <div className="comment-skeleton"><i /><i /></div> : forum.topics.items.map((item) => <button className="country-card" key={item.id} onClick={() => onSelect(item)}><strong>{item.title}</strong><small>{item.messages_count} сообщений</small></button>)}{forum.topics.hasMore && <button onClick={loadMore} disabled={forum.topics.loadingMore}>{forum.topics.loadingMore ? "Загружаем…" : "Показать ещё"}</button>}<div className="topic-reply"><textarea value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Новая тема" /><button onClick={create} disabled={!title.trim()}>Создать</button></div></section>;
}
