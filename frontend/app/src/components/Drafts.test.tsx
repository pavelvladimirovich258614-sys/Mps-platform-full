...
describe("Drafts", () => {
  it("shows a delete control on every draft card", () => {
    render(<Drafts drafts={[draft]} loading={false} error="" onRetry={vi.fn()} onOpen={vi.fn()} onDelete={vi.fn()} />);
  ...
  it("does not delete before the editor confirms", () => {
    ...
    render(<Drafts drafts={[draft]} loading={false} error="" onRetry={vi.fn()} onOpen={vi.fn()} onDelete={onDelete} />);
