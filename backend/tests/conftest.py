...
async def test_app(tmp_path) -> AsyncIterator[FastAPI]:
    ...
    settings = Settings(database_url=f"sqlite+aiosqlite:///{database_path}", jwt_secret="test-secret-key-with-32-characters", auth_bot_token="test-auth-bot-token", relay_bot_token="test-relay-bot-token", managers_chat_id="-100100", lawyer_tg_id="200200", bot_bridge_secret="bridge-secret", telegram_webhook_secret="webhook-test-secret", unisender_go_api_key="key", unisender_from_email="noreply@example.com", minimax_api_key="test-minimax-key", minimax_model="test-model", media_dir=str(tmp_path / "media"))
