...
- F14 задеплоена на production: VPS revision `c549085`, rollback backup `/root/backups/mps-f14-20260822T140321Z`. `mps-backend` restarted and active; frontend rebuilt with verified production VITE values, nginx-readable dist and no localhost API in bundle. Editor/admin теперь видит в основной ленте compact composer с TipTap toolbar: bold, italic, strike, H1–H3, bullet/ordered list, link и blockquote. Создание поддерживает статью и фишку, черновик или публикацию.
