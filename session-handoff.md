...
## Current checkpoint — 2026-09-01, libheif upgrade preflight complete; wheel work in progress
...
4. **Постоянный SEC-HEIF-UPGRADE остаётся `in_progress`.** Свежий VPS preflight подтвердил фактически загруженный `pillow-heif==1.5.0` и bundled `libheif 1.23.1`: `_pillow_heif.cpython-311-x86_64-linux-gnu.so` через RPATH `$ORIGIN/pillow_heif.libs` разрешает ровно `libheif-6b5c61d3.so.1.23.1`; `libheif_info()` также вернул 1.23.1. Системные `libheif1/libheif-dev` не установлены, а Ubuntu 22.04 предлагает только 1.12.0, поэтому системный apt-upgrade не решает задачу. Target wheel: Ubuntu 22.04 x86_64, glibc 2.35, CPython `cp311-cp311`, manylinux_2_27/2_28. Исправленный wheel против libheif 1.23.2 ещё не собран, не установлен и не подтверждён GREEN.
5. SSH host-key false alarm закрыт без ослабления проверки: первоначально сверялся ошибочный IP `82.38.69.27`; правильный `82.39.213.82` трижды с интервалами отдал стабильный ED25519 fingerprint `SHA256:WiGVmbZnCZFtgaCfahdD1IyoUccJ0Si2xFb4TcHxSEE`, уже совпадавший с `known_hosts`. DNS `mir.pod-solncem.ru` разрешился в `82.39.213.82`; BatchMode/IdentitiesOnly/StrictHostKeyChecking с `mps_deploy_key` прошёл. Apt history показывает unattended-upgrade openssh-server `8.9p1-3ubuntu0.14 -> 8.9p1-3ubuntu0.16` 2026-08-07, boot — 2026-08-13; журнал подтверждает update, но причинная связь с регенерацией ключа не заявляется, поскольку у правильного IP несовпадения ключа не было.
6. Свежий pre-closeout checkpoint: local HEAD, `origin/main` и VPS tracked checkout на `b18f9a443aebc9cf3438270468b26e52607f2100`; local и VPS tracked/index changes отсутствуют. На VPS сохранены untracked `.deploy-backups/`, каталог `\\`, `frontend/app/.env.production`, `venv.py310.failed/`; их не удалять и не называть полным clean tree. `mps-backend` active/running под `mps:mps`, ExecStart использует `/opt/mps-platform/venv/bin/uvicorn`, PID 1115264; этот read-only preflight не выполнял install/restart/config/DB changes.
7. **Полный security-аудит по 13 запрошенным категориям завершён как диагностика**, snapshot исходников/production 07566c8: [report.md](D:/mps-platform-full/security-audit-20260831/report.md), [dependencies.md](D:/mps-platform-full/security-audit-20260831/dependencies.md). Отчёты вне git сохранены, не переписаны. Завершение аудита не означает устранение всех находок; C1 только временно mitigated, H1/H2 и остальные замечания открыты. Не заявлялся полный CVE-анализ всей ОС/каждого native codec.
8. Владелец разрешил отдельный документационный коммит **только** feature_list.json, claude-progress.md, session-handoff.md поверх b18f9a4, push и tracker-only fast-forward VPS без нового разрешения. Финальный documentation SHA читается после коммита и сверяется local/origin/VPS; не угадывать его внутри собственного коммита. Application release остаётся 12203dd. Запрещены restart, build, install, миграции, изменения конфигурации/секретов/БД и очистка untracked production artifacts.

## Следующие шаги — приоритет владельца по аудиту

1. **Завершить SEC-HEIF-UPGRADE: custom wheel с bundled libheif 1.23.2 и возврат HEIC.** Продолжить/завершить изолированную сборку target-compatible `cp311` manylinux wheel, зафиксировать provenance и SHA-256, проверить `libheif_info() >= 1.23.2`, `ldd`/RPATH и отсутствие ссылки на 1.23.1. Затем написать и наблюдать RED на восстановление HEIC/HEIF, внести минимальный fail-closed fix, пройти targeted media и полный backend suite с одноразовой migrated PostgreSQL. Только после отдельного согласования выполнить backup/install/restart, health/smoke и live HEIC/JPEG с очисткой. До полного GREEN и production verification сохранять текущий stopgap; rollback не должен сочетать уязвимый wheel с включённым HEIC.
...
3. **Убрать VPS Python 3.11.0rc1 (release candidate).** Выбрать поддерживаемый stable security-patched интерпретатор и создать отдельный воспроизводимый venv вместо in-place замены production.
4. **Устранить 33 расхождения версий пакетов после стабилизации Python.** Использовать согласованные constraints/lock; drift snapshot: [dependency-drift.json](D:/mps-platform-full/security-audit-20260831/dependency-drift.json). Проверить install/pip check, imports, Alembic compatibility, полный backend suite и rollback; не считать локальный GREEN доказательством идентичного production graph.
5. **Medium, затем Low-находки security-аудита.** Разбирать отдельными RED->GREEN scopes в порядке риска/эксплуатируемости, не смешивая их с native wheel, ротацией секрета или окружением Python.
