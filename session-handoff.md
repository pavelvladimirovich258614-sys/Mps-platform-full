...
- F18 local complete, not deployed: two or more adjacent composer images serialize as strict `figure[data-carousel="images"]` and render as an accessible React carousel with previous/next and dots; a single image remains a normal img. Backend nh3 and frontend DOMPurify allow only figure[data-carousel] plus img[src,alt], with class/style/event handlers and extra data attributes removed. Local evidence: RED→GREEN, backend 63 passed, frontend 67 passed and build success. `./init.sh` remains stopped only by the external Hermes charset-normalizer pip-check issue.
