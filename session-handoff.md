...
- F19 local complete, production not deployed: production diagnosis before code proved two temporary JPEG media URLs return 200, published React carousel/CSS mount and Next switches a real 1200x800 image. The reported stacking is composer-only: ImageCarouselNode lacked a NodeView. Composer now uses React NodeViews with a hidden TipTap contentDOM and one visible reusable carousel slide; strict stored `figure[data-carousel="images"]` HTML is unchanged. Standalone/active carousel images have accessible remove controls; deleting from a pair unwraps the remaining img. Toolbar uses a picture SVG; the article-only type select is gone and payload stays `article`. Diagnostic cleanup DELETE 204 and post/media GET 404 completed.
...
1. Дождаться отдельного подтверждения на frontend-only production deploy F19; проверить production VITE values, served bundle и editor/admin carousel preview/delete smoke.
2. Drag-and-drop, paste insertion, reorder и autoplay для изображений остаются будущим отдельным scope без feature ID; не начинать без нового продуктового решения.
