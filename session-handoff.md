...
- **Clean-state-checklist 2026-09-01 (Session 102): 4/6 пунктов ДА, 2/6 НЕТ с известной причиной, не чинить в этой сессии.** Пункт 1 (`./init.sh`) — та же Win32 + global pip-check проблема, не воспроизводилась в этой сессии. Пункт 2 (`pytest`): полный backend без `test_media.py` зелёный (128 passed / 10 skipped); 6 тестов в `test_media.py` падают RED, потому что локальный `pip show pillow_heif` указывает на старый wheel `1.5.0` (старый vulnerable), а не на production-target `1.5.0+libheif1.23.2` (установлен только в production venv через `--no-deps` от staging-wheel). На production все HEIC-пути подтверждены живыми загрузками (Session 102 шаг 9): HEIC 200, JPEG 200, disguise 200, malformed 422, все 6 variants decoded, cleanup remaining=0. Фикс для следующей сессии: установить в локальный venv тот же кастомный wheel из `D:/mps-platform-full/heif-wheel-build-20260901/dist/pillow_heif-1.5.0+libheif1.23.2-cp311-cp311-manylinux_2_27_x86_64.manylinux_2_28_x86_64.whl` (или публичный GitHub release по `backend/requirements-heif.lock`); вне scope закрытия SEC-HEIF-UPGRADE.
