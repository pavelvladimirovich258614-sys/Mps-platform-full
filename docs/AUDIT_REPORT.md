...
**Статус: открыто 2026-08-20.** В I-20 добавлены UI-чекбоксы и ссылки на policy у подписки, отзывов и Q&A, но backend не хранит версию/время/доказательство согласия. CookieBanner показывает факт cookies, policy и кнопку явного согласия при первом визите, однако стартовые API-запросы приложения выполняются до выбора пользователя. Полное соответствие 152-ФЗ, включая правовое основание, доказуемый consent, категории cookies и блокирование необязательной обработки до выбора, требует отдельного продуктового и архитектурного решения.
