...
## Известная production-инфраструктурная проблема — UnisenderGo недоступен с VPS

**Статус: открыто, внешний сетевой блокер (2026-08-21).** Email-код и digest-рассылка не работают на production, потому что VPS не может установить исходящее TCP-соединение на порт 443 к адресам Unisender `31.184.200.*` (в том числе `goapi.unisender.ru` и `go1.unisender.ru`). `curl -v --max-time 10` к обоим hosts завершился `ConnectTimeout` до HTTP/TLS; backend повторяет это как `httpx.ConnectTimeout` и честный API `502`.

**Подтверждённые границы:** локальный UFW разрешает исходящий трафик, `iptables OUTPUT` имеет policy `ACCEPT` без явных DROP/REJECT для этих адресов или 443. В то же время `ya.ru` (российский IP) и `google.com` успешно проходят TCP, TLS и HTTP. Следовательно, это не ошибка payload, `goapi`/`go1`, frontend или общий запрет российского HTTPS; это селективная внешняя фильтрация/маршрутизация до сети Unisender. Код использует `goapi` default и `X-API-KEY`, но валидность ключа и sender domain нельзя подтвердить до восстановления TCP-связности.

**Требуется:** открыть тикет HostKey с временем и адресами `31.184.200.*:443`; при необходимости параллельно проверить фильтрацию у Unisender. Альтернативы — SMTP-транспорт UnisenderGo либо другой email-провайдер. Не менять firewall VPS без отдельного согласования.
...
**Статус: закрыто 2026-08-20.** Шесть публичных реквизитов, включая ОГРН, хранятся в settings; миграция `20260820_0008` заполняет официальные значения на чистой БД. `GET /api/v1/settings/public` возвращает только whitelist, без CTA/Irishka. Footer, About и privacy page используют эти settings; формы имеют UI-согласие и ссылку на policy. Полная consent/cookie архитектура вынесена в I-21.
...
### I-21. Доказуемое согласие и управление cookies требуют отдельного решения

**Статус: отложено до pre-launch юридической проверки 2026-08-21.** Накопленных пользовательских данных нет; аналитика, реклама и tracking cookies не используются. Базовая гигиена уже есть: policy, публичные реквизиты оператора и UI-согласие у форм. Перед публичным запуском после консультации с юристом рекомендуемый вариант — backend-хранение `consent_given_at` и `consent_version`; также нужно отдельно определить правовое основание и политику для стартовых API-запросов и cookies.

**Объём:** отдельная сессия.
