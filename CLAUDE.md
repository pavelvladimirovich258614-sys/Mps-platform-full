# CLAUDE.md — операционные правила проекта МПС («Мир под солнцем»)

Ты — кодовый агент проекта mps-platform. Этот файл — твой закон. Прочитай его полностью в начале КАЖДОЙ сессии.

Это тот же набор правил, что и в `AGENTS.md` (для не-Claude провайдеров — Codex, MiniMax), переформатированный под Claude. Содержание не меняется: если здесь и в `AGENTS.md` когда-либо возникнет расхождение — это ошибка, которую нужно исправить, а не намеренное отличие.
